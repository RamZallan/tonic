...
class MachineCard extends Component {
    render() {
        ...
        const statusBadge =
            this.props.machine.is_online
            ? (<Badge className="float-right" color="success">Online</Badge>)
            : (<Badge className="float-right" color="danger">Offline</Badge>);
        return (
                <Col xs="12" lg={isSnack ? '12' : '6'}>
                    <Card>
                        <CardHeader>{this.props.machine.display_name} {statusBadge}</CardHeader>
                        <CardBody>
